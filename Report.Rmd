...
###############################################################################
# from here we get the list of player and now we scrap the data in a such way that we can store then together for further use 
country <- list()
...
This table shows data of previous year
...
# This graph plots out the win to loss ratio of each country which played ODI Cricket in the selected timeframe, with their position on the world map.
...
# The Data is saved in descending order, and we only need data for teams
# which have played ODI Cricket.
...
t1 <- (t %>% 
         filter((as.integer(year(t$'Match Date')) >= 1971) & (as.integer(year(t$'Match Date')) <= 2023)))
...
-   **Plot**: Users can see how much the team is scoring over a specified time frame.
```{r}
#| echo: false
#| eval: true
...
t <- "Australia"
tf <- team_filter(t)
tab <- tf %>% filter((as.integer(year(tf$'Match Date')) <= 2023) & (as.integer(year(tf$'Match Date')) >= 1971))
Date <- NULL
Score <- NULL
Win <- NULL
...
# This plots out the scores made by the selected team, and colour codes it according to whether it won or lost the match.
...
t <- t2 %>% 
  filter((as.integer(year(t2$'Match Date')) >= 1971) & (as.integer(year(t2$'Match Date')) <= 2023))
...
for (i in 1:length(other)) {
  ...
  Won <- append(Won, 
                length((t1 %>% filter(Winner == team))$Winner))
  Won_Bat <- append(Won_Bat, 
                    length((t1 %>% 
                              filter((Winner == team) & (Winning_Innings == "Batting")))$Winner))
  Won_Bowl <- append(Won_Bowl, 
                     length((t1 %>%
                               filter((Winner == team) & (Winning_Innings == "Bowling")))$Winner))
  Lost_Bat <- append(Lost_Bat, 
                     length((t1 %>% 
                               filter((Winner == other[i]) & (Winning_Innings == "Bowling")))$Winner))
  Lost_Bowl <- append(Lost_Bowl,
                      length((t1 
                              %>% filter((Winner == other[i]) & (Winning_Innings == "Batting")))$Winner))
  Lost <- append(Lost,
                 length((t1 %>% filter(Winner == other[i]))$Winner))
}
...
![](pictures/MatchUpInput.png){width="300"}
-   **Year Range Slider**: Users can specify the range of years they want to analyze.
...
# This plots out scores made by both teams playing against each other, colour coded, by which team scored the runs.
...
tab <- tf %>%
  filter((as.integer(year(tf$'Match Date')) <= y2) & (as.integer(year(tf$'Match Date')) >= y1)) %>%
  filter(Opponent == t2)
Team_1_Batting <- length((tab %>% filter(Winner == t1) %>% 
                            filter(Winning_Innings == "Batting"))$Winner)
Team_1_Bowling <- length((tab %>% filter(Winner == t1) %>% 
                            filter(Winning_Innings == "Bowling"))$Winner)
Team_2_Batting <- length((tab %>% filter(Winner == t2) %>% 
                            filter(Winning_Innings == "Batting"))$Winner)
Team_2_Bowling <- length((tab %>% filter(Winner == t2) %>%
                            filter(Winning_Innings == "Bowling"))$Winner)
...
# Shows how many matches which team won, and in which innings.
tb
```
...
# This plots out the correlation of Happiness Index of a country with its Win to Loss Ratio.
...
-   **Country-Specific View**: This view can be used to visualise the variation of Happiness Index and Win to Loss Ratio of the selected country with year.
    ...
    # This plots out Happiness Index and Win To Loss Ratio with respect to Year, of the seleted country.
...
-   **General View**: This view is to show the correlation of GDP with the Win to Loss Ratio of the selected countries.
    ...
    # This plots out the correlation of GDP of a country with its Win to Loss Ratio.
    barplot(names = Countries, height = Correlation, xlab = "Countries",
            ylab = "Correlation")
    ```
...
for (i in Years) {
  GDP <- append(GDP, as.integer(tab[i - 1957]))
  Win <- length((Tf %>% filter(Winner == t1) %>% 
                   filter(year(`Match Date`) == i))$Winner)
  Loss <- length((Tf %>% filter(Loser == t1) %>%
                    filter(year(`Match Date`) == i))$Loser)
  if (Loss == 0) {
    Win_To_Loss <- append(Win_To_Loss, Win)
  }
...
# This plots out GDP and Win To Loss Ratio with respect to Year, of the seleted country.
matplot(Years, cbind(GDP, 1000*Win_To_Loss), col = c("red", "blue"),
        pch = c(13, 16), xlab = "Years", ylab = "GDP + Win To Loss Ratio(x 1000)")
legend("topright", legend = c("GDP per Capita", "Win To Loss Ratio(x 1000)"),
       col = c("red", "blue"), pch = c(13, 16))
```
