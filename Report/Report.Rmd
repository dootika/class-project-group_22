...
so after scraping we croos check the names of a player mainually , after getting the correct name we formed the list and makes a function the scrap the data a give input as a player name and takes output the dataset of the past years . The GDP per Capital data was initially downloaded from - [kaggle](https://www.kaggle.com/datasets/zgrcemta/world-gdpgdp-gdp-per-capita-and-annual-growths?select=gdp_per_capita.csv). It is cleaned to be used with the dataset collected. The Happiness Index data was downloaded from - [kaggle](https://www.kaggle.com/datasets/simonaasm/world-happiness-index-by-reports-2013-2023/?select=World+Happiness+Index+by+Reports+2013-2023+with+nulls.csv).
...
-   ODI Player data tab in this tab viwer have to give input as a country name and player name and player data will be shown
...
-   ODI team data tab by selection the year viwer can see the relevent output in the side as a tibble as a as plot and visualisation also .

    ![](images/World_W_L.png)

-   Match tab In this tab player have to select the team baisd on that output will be genereted by using the ***Cricket_Match*** and viwer can also define a new team by selecting new team .
...
This plot shows the win looss ratio of the team which is in coloured country on map and density of colour shows the high win loosse ratio .
...
This plot shows the result of previous year matches x axis is the year and y axis shows the score and red and skyblue point are for different country .
...
This plot shows the combine view of happines index and winloos ratio .
...
This plot shows the correclation between a fix country and other country , here India is fixed . \#### 5.4.6 A ggplot showing batsman output visualisation of match output
...
This plot shows the result of batsman as visualisation x bar shows the name of player and y bar shows the total run hit by a player shape shows who has taken the wicket , color shows no of Four, size shows No of Six. This plot is in ploty but in pdf cant be shown so to see deiscribed plot please refer to html document .
